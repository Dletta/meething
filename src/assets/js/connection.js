...
export default class Connection {
  ...
  establish () {
    ...
    let damSocket,
      metaData, 
      socketId;
    med.damSocket = damSocket = new DamEventEmitter(this.mediator.root, this.mediator.room);
    let commElem = document.getElementsByClassName("room-comm");
      ...
      med.socketId = socketId = med.h.uuidv4();
      med.damSocket.on("postauth",function(auth){
        this.initPresence();
        med.metaData = metaData = new MetaData(med.root, med.room, med.socketId, this.metaDataReceived.bind(this));
      ...
      med.damSocket.on('SDP', function (data) {
        try {
          ...
          if (data.to !== med.socketId) {
            if (med.DEBUG) console.log("not for us? dropping sdp", data.to, med.socketId);
            return;
          }
        } catch (e) {
          console.log(e, data);
  ...
  initPresence () {
    ...
    med.presence = new Presence(root, room);
  ...
  pcPartnerName.ontrack = e => {
   // let med = this.mediator;
    ...
    if (el) {
      ...
      var el = med.h.addVideo(partnerName,str);
      med.h.setVideoSrc(el,str);
    }
  };
  ...
  pcPartnerName.onconnectionstatechange = d => {
    ...
    switch (pcPartnerName.iceConnectionState) {
      case "connected":
        ...
        med.metaData.sendControlData({ username: username, id: med.socketId, online: true });
        break;
      case "disconnected":
  ...
  setMediaBitrates (sdp) {
    return sdp;
    let med = this.mediator;
    if (med.videoBitrate == 'unlimited' || !this.calculateBitrate()) {
      console.log("Not changing bitrate max is set")
      return sdp;
    } else {
      return this.setMediaBitrate(this.setMediaBitrate(sdp, "video", med.videoBitrate), "audio", 50);
    }
  }
  ...
  calculateBitrate () {
    ...
    if (oldBitrate == med.videoBitrate) {
      ...
      this.sendMsg("Bitrate " + med.videoBitrate, true);
      return true;
    }
  }
